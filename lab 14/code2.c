...
int main(void)
{
	...
	*HEX5_HEX4_ptr = 0;								// clear the display
	...
	while (1)
	{
		...
		*HEX5_HEX4_ptr = seg7[time/10] << 8;
        *HEX5_HEX4_ptr |= seg7[time%10];
		*HEX3_HEX0_ptr = seg7[time/10] << 24;
        *HEX3_HEX0_ptr |= seg7[time%10] << 16;
		if (!flag)
			time+=1;
		if (time >100) time = 1;
